...
app = FastAPI(
    title="面向企业制度知识库的 RAG 检索增强与幻觉治理优化",
...
@app.post("/rewrite/decompose")
def decompose(request: AskRequest):
    try:
        result = rag.decompose(request.question)
        return result
    except Exception as e:
        logger.error(f"decompose失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def ask(request: AskRequest):
    ...
    if not request.question.strip():
        raise HTTPException(status_code=400, detail="question 不能为空")
    try:
        return rag.answers(request.question)
    except Exception as e:
